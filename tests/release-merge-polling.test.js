...
const workflow = readFileSync(
  new URL("../.github/workflows/release-prepare.yml", import.meta.url), "utf8",
);
...
function runScenario(t, states, { mergeExit = 0, timeout = 10 } = {}) {
  const dir = mkdtempSync(join(tmpdir(), "gpu-shared-release-merge-"));
...
test("immediate merge still requires observed MERGED state", (t) => {
  const result = runScenario(t, ["MERGED"]);
  assert.equal(result.status, 0, result.stderr);
  assert.ok(result.state.reads >= 1);
  assert.match(result.stdout, /RELEASE_CONTINUES/u);
});

test("successful queue acceptance waits through OPEN until MERGED", (t) => {
  const result = runScenario(t, ["OPEN", "OPEN", "MERGED"]);
  assert.equal(result.status, 0, result.stderr);
  assert.ok(result.state.reads >= 3);
  assert.match(result.stdout, /RELEASE_CONTINUES/u);
});

test("initial merge failure continues checking until eventual merge", (t) => {
  const result = runScenario(t, ["OPEN", "MERGED"], { mergeExit: 1 });
  assert.equal(result.status, 0, result.stderr);
  assert.ok(result.state.reads >= 2);
});

for (const state of ["CLOSED", "API_ERROR", "UNKNOWN"]) {
  test(`${state} fails closed even when merge command succeeds`, (t) => {
    const result = runScenario(t, [state]);
...
test("queued merge fails at bounded deadline without releasing", (t) => {
  const result = runScenario(t, ["OPEN"], { timeout: 2 });
  assert.notEqual(result.status, 0);
  assert.doesNotMatch(result.stdout, /RELEASE_CONTINUES/u);
