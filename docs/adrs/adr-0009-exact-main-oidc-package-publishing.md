# ADR-0009: Exact-main OIDC package publishing
...
The former `@plasius/gpu-shared` workflow prepared a release commit and published it from
...
has an immutable, independently reviewable identity. The package remains browser-safe and framework-independent; release automation does not widen its runtime boundary.
...
`Plasius-LTD`, repository `gpu-shared`, workflow `cd.yml`, environment
`production`, and action `npm publish`. GitHub `main` and `production`
policies are independent admission controls. The inherited product flag is `feedback.game-diagnostics.enabled`; the release-governance control is `platform.public-artifact-integrity.enabled` and introduces no runtime exposure. Rollback disables
